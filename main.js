...
function showCouponModal() {
    ...
    if (window.applyStoredTranslation) window.applyStoredTranslation();
...
document.addEventListener('click', function (e) {
    ...
    if (openBtn) {
        ...
        var planAccents = {
            'Aula Experimental': { color: '#C0C0C0', text: '#000', shimmer: 'rgba(192,192,192,.3)' },
            ...
            'Ebook': { color: '#00F3FF', text: '#000', shimmer: 'rgba(0,243,255,.3)' }
        };
        ...
        var installmentText = document.getElementById('modal-installment-text');
        if (installmentText) {
            installmentText.style.display = (installments === 'Pagamento Único' || plan === 'Aula Experimental') ? 'none' : 'block';
        }
        ...
        if (plan === 'Aula Experimental' || plan === 'Coach Gratuito') {
...
if (typeof Swup !== 'undefined') {
    ...
    swup.hooks.on('content:replace', function () {
        ...
        updateActiveLink();
        window.scrollTo(0, 0);
        if (window.applyStoredTranslation) {
...
document.addEventListener('DOMContentLoaded', () => {
    const opggTrigger = document.getElementById('opgg-trigger');
    const opggModal = document.getElementById('opgg-modal');
    const opggClose = document.getElementById('close-opgg-modal');

    if(opggTrigger && opggModal && opggClose) {
        opggTrigger.addEventListener('click', () => {
            opggModal.classList.add('active');
            document.body.style.overflow = 'hidden';
            
            // Apply vanilla tilt to the new modal cards dynamically if library is present
            if(window.VanillaTilt) {
                VanillaTilt.init(document.querySelectorAll("#opgg-modal .proof-card"), {
                    max: 10,
                    speed: 400,
                    glare: true,
                    "max-glare": 0.2,
        ...
        opggClose.addEventListener('click', () => {
            opggModal.classList.remove('active');
            document.body.style.overflow = '';
        });

        opggModal.addEventListener('click', (e) => {
            if (e.target === opggModal) {
                opggModal.classList.remove('active');
                document.body.style.overflow = '';
            }
        });
...
// Fullscreen Image Lightbox (Resultados)
document.addEventListener('click', function(e) {
    if (e.target.closest('.result-card img')) {
        ...
        if (!lightbox) {
            ...
            lightbox.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.95);backdrop-filter:blur(10px);z-index:999999;display:flex;justify-content:center;align-items:center;opacity:0;transition:opacity 0.3s ease;cursor:pointer;';
            
            const LB_img = document.createElement('img');
            LB_img.id = 'global-lightbox-img';
            LB_img.style.cssText = 'max-width:95%;max-height:95vh;border-radius:12px;transform:scale(0.8);transition:transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);box-shadow:0 15px 50px rgba(0,0,0,0.9), 0 0 20px rgba(225, 6, 0, 0.2);';
            
            const LB_close = document.createElement('div');
            LB_close.innerHTML = '<i class="fas fa-times"></i>';
            LB_close.style.cssText = 'position:absolute;top:25px;right:35px;color:#fff;font-size:2rem;cursor:pointer;transition:color 0.3s;';
            LB_close.onmouseover = () => LB_close.style.color = '#E10600';
            LB_close.onmouseleave = () => LB_close.style.color = '#fff';

            lightbox.appendChild(LB_img);
            lightbox.appendChild(LB_close);
            document.body.appendChild(lightbox);
